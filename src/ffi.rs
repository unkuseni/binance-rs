...
extern "C" fn dummy(_: *const c_char) -> *mut c_char {
    std::ptr::null_mut()
}

static mut FUNC_CPP_FROM_RUST: extern "C" fn(s: *const c_char) -> *mut c_char = dummy;
...
#[no_mangle]
pub extern "C" fn init_from_cpp(callback: extern "C" fn(_: *const c_char) -> *mut c_char) -> i32 {
    unsafe {
...
#[no_mangle]
pub async extern "C" fn ws_order_book_rs(
    symbol: *const c_char, callback: extern "C" fn(_: *const c_char) -> *mut c_char,
) -> i32 {
...
#[no_mangle]
pub async extern "C" fn ws_agg_trade_rs(
    symbol: *const c_char, callback: extern "C" fn(_: *const c_char) -> *mut c_char,
) -> i32 {
    let callback_fn = |event: FuturesWebsocketEvent| {
        callback(CString::new(format!("{:?}", event)).unwrap().into_raw() as *const c_char);
...
#[no_mangle]
pub async extern "C" fn ws_mark_price_rs(
    symbol: *const c_char, callback: extern "C" fn(_: *const c_char) -> *mut c_char,
) -> i32 {
    let callback_fn = |event: FuturesWebsocketEvent| {
        callback(CString::new(format!("{:?}", event)).unwrap().into_raw() as *const c_char);
...
pub async extern "C" fn cancel_order_with_client_id_rs(
    symbol: *const c_char, orig_client_order_id: *const c_char,
) -> *mut c_char {
    unsafe {
        ...
        let res = format!(
            ...
            ACCOUNT
                .as_mut()
                .unwrap()
                .cancel_order_with_client_id(rs_symbol, rs_orig_client_order_id)
                .await
        );
...
pub async extern "C" fn cancel_order_rs(
    symbol: *const c_char, order_id: *const c_char,
) -> *mut c_char {
    unsafe {
        let rs_symbol = CStr::from_ptr(symbol).to_str().unwrap();
        ...
        let res = format!(
            ...
            ACCOUNT
                .as_mut()
                .unwrap()
                .cancel_order(rs_symbol, rs_order_id.parse::<u64>().unwrap())
                .await
        );
        CString::new(res).unwrap().into_raw() as *mut c_char
    }
...
pub async extern "C" fn custom_order_rs(
    symbol: *const c_char, qty: *const c_char, price: *const c_char, stop_price: *const c_char,
    order_side: *const c_char, order_type: *const c_char, time_in_force: *const c_char,
    activation_price: *const c_char, callback_rate: *const c_char, close_position: *const c_char,
) -> *mut c_char {
    unsafe {
        let rs_symbol = CStr::from_ptr(symbol).to_str().unwrap();
        ...
        let res = format!("{:?}", ACCOUNT.as_mut().unwrap().custom_order(order).await);
        CString::new(res).unwrap().into_raw() as *mut c_char
    }
}
...
// pub fn custom_order<S, F>(&self, symbol: S, qty: F, price: f64, stop_price: Option<f64>, order_side: OrderSide,
//    order_type: OrderType, time_in_force: TimeInForce, new_client_order_id: Option<String>, ) -> Result<Transaction>
pub async extern "C" fn batch_orders_rs(
    ...
    time_in_forces: &[*const c_char], activation_prices: &[*const c_char],
    callback_rates: &[*const c_char], close_positions: &[*const c_char],
) -> *mut c_char {
    unsafe {
        ...
        let res = format!(
            ...
            ACCOUNT
                .as_mut()
                .unwrap()
                .custom_batch_orders(count, orders)
                .await
        );
        CString::new(res).unwrap().into_raw() as *mut c_char
    }
...
pub async extern "C" fn exchange_info_rs() -> *mut c_char {
    unsafe {
        let res = format!("{:?}", GENERAL.as_mut().unwrap().exchange_info().await);
        CString::new(res).unwrap().into_raw() as *mut c_char
    }
}
...
pub async extern "C" fn get_custom_depth_rs(
    symbol: *const c_char, depth: *const c_char,
) -> *mut c_char {
    unsafe {
        let rs_symbol = CStr::from_ptr(symbol).to_str().unwrap();
        ...
        let res = format!(
            ...
            MARKET
                .as_mut()
                .unwrap()
                .get_custom_depth(rs_symbol, rs_depth.parse::<u64>().unwrap())
                .await
        );
        CString::new(res).unwrap().into_raw() as *mut c_char
    }
...
pub async extern "C" fn get_price_rs(symbol: *const c_char) -> *mut c_char {
    unsafe {
        let rs_symbol = CStr::from_ptr(symbol).to_str().unwrap();

        let res = format!("{:?}", MARKET.as_mut().unwrap().get_price(rs_symbol).await);
        CString::new(res).unwrap().into_raw() as *mut c_char
    }
}
...
pub async extern "C" fn get_book_ticker_rs(symbol: *const c_char) -> *mut c_char {
    unsafe {
        let rs_symbol = CStr::from_ptr(symbol).to_str().unwrap();

        let res = format!(
            "{:?}",
            MARKET.as_mut().unwrap().get_book_ticker(rs_symbol).await
        );
        CString::new(res).unwrap().into_raw() as *mut c_char
    }
}
